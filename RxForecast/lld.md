...
- **Content Safety groundedness gate (firmed up 2026-08-12, previously "optional"):** every Substitution Reasoner output runs through **Azure AI Content Safety's groundedness detection API** before it reaches the self-consistency safety pass (§4.3 SAFE2 node) — checking the generated rationale text against the actual grounding documents retrieved from `formulary-kb` (Orange Book TE code, contract record, DEA schedule) for that call. A flagged response is treated exactly like a SAFE2 discrepancy: routed to pharmacist review, never silently corrected or auto-retried into the buyer's queue. **Not a replacement** for the PRD's hardcoded clinical safety rules (no Schedule II auto-sub, etc.) — those remain deterministic code, not a content-safety classifier, because a probabilistic filter is the wrong tool for a rule that must never have exceptions. This is specifically aimed at the failure mode the hardcoded rules *can't* catch: a fabricated NDC, dosage, or contract price inside an otherwise-well-formed rationale.
- **Continuous production evaluation (Azure AI Foundry Evaluation SDK) — new, complements rather than replaces the offline harness:** `PRD.md` §8's offline harness (forecast MAPE, 200-case pharmacist-labeled substitution set) stays the system of record for whether the model is *good* — that judgment is domain-specific and was never going to be off-the-shelf. What Foundry's Evaluation SDK adds is a layer the offline harness structurally can't provide: **continuous, per-call groundedness/relevance scoring in production**, sampled from live Substitution Reasoner and Shortage Watcher traffic, not a periodic batch run. Practically: every LLM call's `(prompt, retrieved context, completion)` triple is scored async against Foundry's built-in groundedness and relevance evaluators, written to `metric_snapshots` under the `ai_quality` category (`engg.md` FEATURE_9) as a nightly rollup, and any single-call score below a threshold is logged for the weekly HHH_Eval review (`PRD.md` §8) rather than triggering a runtime action — it's a monitoring signal, not a second gate on top of the Content Safety check above.
- **Agent tracing as the audit-trail's engineering counterpart:** Foundry's built-in agent tracing (prompt, completion, token usage, latency, and now the Content Safety score per call) is captured with a `trace_id` per LLM call. That `trace_id` is threaded into the corresponding `StoreRunState.citations` entry and, once a substitution reaches a buyer decision, into the `audit_log` row's `sources[]` (`engg.md` FEATURE_7) — so a compliance reviewer looking at *why* a substitution was recommended can pull the full Foundry trace for that exact call, not just the citation text. Foundry's trace itself still lives in Azure Monitor/Log Analytics as an engineering-facing debugging/cost-audit layer; only the `trace_id` pointer crosses into the compliance-facing audit trail, not the raw trace content.
- **What doesn't change:** the hard safety rules, `an-critical` UI treatment, human-in-the-loop gates, and the offline harness's role as the actual go/no-go judgment are all unchanged — none of that logic lives in "which cloud serves the model."

**Cost note:** `PRD.md` Appendix B priced Claude usage against Anthropic's direct API rates. Azure AI Foundry's Claude pricing may carry a small platform premium, may match it exactly, or may differ by deployment type (pay-as-you-go vs. provisioned throughput) — **this needs a confirmed number from Azure's current pricing page at build time**, not an assumed match. The Content Safety resource and Evaluation SDK usage are additional, separately-metered Azure costs not yet reflected in Appendix B — flagged as a new open item, not folded into the existing Foundry/Claude line.
...
8. **Content Safety groundedness threshold (added 2026-08-12)** — §4.5's groundedness gate needs a confirmed pass/fail score cutoff, tuned against real Substitution Reasoner output once the Foundry project is live; too strict over-routes to pharmacist review and erodes the buyer-accept-rate metric, too loose defeats the point of the gate. Start conservative in the pilot, tune down as false-positive rate is observed.
9. **Evaluation SDK sampling rate (added 2026-08-12)** — §4.5's continuous production evaluation scores a sample of live traffic, not 100% of calls, to bound the added Foundry API cost; the actual sampling rate (assume 100% during pilot given low volume, revisit at scale) needs to be set once real per-call Evaluation SDK pricing is confirmed alongside item 7's re-pricing work.
